...
function LapTimeChart({ lapData = [], currentLap = 0, raceResults = [] }) {
  ...
  // Get top 10 drivers from race results for cleaner chart
  ...
  // Convert lap times to seconds for graphing
  const timeToSeconds = (timeString) => {
    if (!timeString) return null;
    
    // Format can be "1:23.456" or "1:23:456"
  ...
  for (let i = 0; i < lapsToShow; i++) {
    ...
    topDrivers.forEach(driver => {
      ...
      if (timing && timing.time) {
        lapEntry[driver.driverId] = timeToSeconds(timing.time);
      }
    });
  ...
  function getDriverColor(position) {
    const colors = [
      '#ff1801', '#00d4ff', '#00ff88', '#ffd700', '#ff00ff',
      '#00ffff', '#ff6b00', '#00ff00', '#ff0088', '#8800ff'
    ];
          ...
          {payload.map((entry, index) => {
            ...
            return (
              <p key={index} style={{ 
                ...
                {driver?.code}: {entry.value?.toFixed(3)}s
              </p>
            );
          })}
        </div>
  ...
  return (
    <div style={{ 
      ...
      <ResponsiveContainer width="100%" height={300}>
        <LineChart data={chartData}>
          ...
          <YAxis 
            ...
            label={{ value: 'Lap Time', angle: -90, position: 'insideLeft', fill: '#666' }}
          />
          <Tooltip content={<CustomTooltip />} />
          <Legend 
            wrapperStyle={{ paddingTop: '20px' }}
            formatter={(value) => {
              const driver = topDrivers.find(d => d.driverId === value);
              return driver?.code || value;
            }}
          />
      ...
      <div style={{ 
        ...
        Showing lap times for top 10 finishers
      </div>
    </div>
  );
}
